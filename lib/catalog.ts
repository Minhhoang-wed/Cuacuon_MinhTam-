...
import type {
  ...
  CatalogService,
  CatalogStoreBranch,
  CatalogServiceDistrict,
  CatalogServicePriceItem,
  CatalogServicePriceCategory,
  CatalogArticle,
  ManagedSiteConfig,
  HomepageContent,
  AboutPageContent,
...
function mapArticle(row: DbArticle): CatalogArticle {
  ...
  return {
    ...
    author: row.author || "Kỹ Thuật Viên Minh Tâm",
    isFeatured: row.is_featured,
    status: row.status,
    sortOrder: row.sort_order || 0,
...
export async function getSiteSettings(): Promise<ManagedSiteConfig> {
  ...
  try {
    ...
    return {
      ...
      address: row.branch_1_address || row.address || fallback.address,
      branch1Name: row.branch_1_name || "Cơ sở 1 (Trụ sở Quận 10)",
      branch1Address: row.branch_1_address || row.address || "361 Lý Thường Kiệt, P. Tân Hòa, Quận 10, TP.HCM",
      branch2Name: row.branch_2_name || "Cơ sở 2 (Chi nhánh Quận 6)",
