...
import { getCategories, getHomepageContent, getProducts, getSiteSettings } from "@/lib/catalog";
...
export async function getAdminOverview() {
  if (!getSupabaseConfig()) {
    ...
    return { products: products.length, published: products.filter((item) => item.status === "published").length, drafts: 0, categories: categories.length, projects: 0, media: 0 };
  }
  const token = await getAdminAccessToken();
