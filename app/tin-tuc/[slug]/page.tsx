...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const slug = (await params).slug; const item = getArticle(slug); if (!item) return {}; return { title: item.title, description: item.excerpt, alternates: { canonical: `/tin-tuc/${slug}` }, openGraph: { type: "article", title: item.title, description: item.excerpt, images: ["/og.png"] }, twitter: { card: "summary_large_image", title: item.title, description: item.excerpt, images: ["/og.png"] } }; }
